...
# Source Code Github
https://github.com/vido21/Machine-Learning/blob/main/Tugas%20Image%20Classifier%20-%20Multi%20Layer%20Perceptron%20Final/tugas_image_classifier_multi_layer_perceptron_final.py
...
imageDimensions = (320, 240, 1)

def myModel():
    model = Sequential()
    model.add((Conv2D(64, (5, 5), input_shape=imageDimensions, activation='relu')))
...
"""Potongan script di atas merupakan arsitektur dari CNN dengan menggunakan 2 convolution layer awal dengan 64 filter. Input merupakan iamge dengan dimensi 320 x 240 grayscale. Kernel size yang digunakan pada kedua layer pertama adalah 5 x 5. Kemudian terdapat 1 layer pooling dengan ukuran 2 x 2 yang diikuti 2 layer convolution. Dua layer convolution dengan 32 filter dengan kernel size 3 x 3. Terdapat pooling layer terakhir dengan size 2 x 2. Kemudian digunakan regularisasi dengan lamda 0.5. Kemudian feature tersebut di-flatten untuk menjadi input pada hidden layer berikutnya. Hidden layer didefinisikan sebanyak 512 neuron dengan fungsi aktivasi relu. Setelah itu dilakukan regularisasi dengan nilai lamda sama dengan 0.5. Kemudian terdapat output layer sebanyak 3 neuron dengan fungsi aktivasi softmax.

Kemudian arsitektur ini akan diimplementasikan dengan optimizer adam dengan learning rate 0.001, loss function categorical crossentropy dan parameter yang dievaluasi adalah accuracy.
"""

# Add validation set
X_train, X_validation, Y_train, Y_validation = train_test_split(X_train, Y_train, test_size = 0.2)

"""Potongan script di atas digunakan untuk splitting data training menjadi data training dan data validasi dengan ratio 0.2 untuk data validasi."""
...
"""Script di atas digunakan untuk memfilter/menghilangkan bias yang telah diinisiasi pada model sebelumnya."""

# Change shape to fit the model
...
"""Potongan script di atas bertujuan untuk reshaping image/data agar bisa diproses pada arsitektur yang telah didefinisikan."""
...
"""Potongan script di atas merupakan bagian untuk melakukan training dengan nilai epoch yang telah diinisiasi yaitu 50.

Dari hasil di atas dapat diketahui bahwa model menghasilkan accuracy yang sangat baik pada training yang bisa dilihat dari nilai accuracy yang sangat baik dan loss function yang sangat rendah. Namun, pada data validasi nilai accuracy yang didapatkan hanya berkisar pada rentang 0.4 - 0.5 dengan loss function yang cukup tinggi pada rentang 3 - 4.
"""
...
"""Script di atas digunakan untuk memplot loss function dan akurasi dari data training dan validation. Kemudian terdapat bagian untuk melihat hasil akurasi dari data test.
Data output di atas terlihat bahwa nilai akurasi yang didapatkan pada data validasi dan data test masih sangat buruk. Didapatkan nilai test akurasi dari data test sebesar 50%.

Dari kedua hasil di atas dapat diketahui bahwa model yang telah merupakan model yang overfit. Hal ini bisa dilihat dari baiknya nilai akurasi pada data train tetapi nilai akurasi pada data validasi dan data test justru sangat buruk.

# Perbandinga model MLP dan CNN

Pada model MLP yang telah dibuat baik dengan menggunakan learning rate 0.1 dan 0.8 keduanya hanya mendapatkan nilai akurasi sebesar 30%. Sedangkan pada model yang menggunakan CNN akurasi yang didapat sekitar 50%. Dari nilai akurasi yang diperoleh ini dapat disimpulkan bahwa model yang menggunakan CNN jauh lebih baik dibanding model MLP yang telah dibuat. Meskipun model yang menggunakan CNN mempunyai akurasi yang lebih baik, model ini masih mengalami overfit sehingga bisa dikatakan model yang dibuat belum cukup baik.
"""